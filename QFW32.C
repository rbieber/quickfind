...
int  PASCAL KeyInput(void)
{
   ...
   do
      ...
      if (dwNumChars == 1 && input.EventType == KEY_EVENT && input.Event.KeyEvent.bKeyDown)
         break;
      }
   while (TRUE);
...
unsigned _dos_findfirst(const char *pszPathName, unsigned attrib, struct _find_t *pfind_t)
{
   ...
   pfind_t->search_handle = _findfirst((char *) pszPathName, (struct _finddata_t *) pfind_t);
...
unsigned _dos_findnext(struct _find_t *pfind_t)
{
   ...
   if (pfind_t->search_handle && pfind_t->search_handle != -1)
      usReturn = _findnext(pfind_t->search_handle, (struct _finddata_t *) pfind_t);
